...
export const COGNATE_SETS: CognateSet[] = [
  ...
  { root: '*wḗdr̥', meaning: '水/Water', languages: { '英语': 'water', '法语': 'onde', '德语': 'Wasser', '西班牙语': 'onda', '俄语': 'вода', '拉丁语': 'unda' }, period: 'PIE', family: 'ie' },
  ...
  { root: '*ǵneh₃-', meaning: '知道', languages: { '英语': 'know', '法语': 'connaître', '德语': 'kennen', '西班牙语': 'conocer', '俄语': 'знать', '拉丁语': 'gnoscere' }, period: 'PIE', family: 'ie' },
  ...
  { root: '*gʷen-', meaning: '女人', languages: { '英语': 'queen', '法语': 'femme', '德语': 'Frau', '西班牙语': 'mujer', '俄语': 'жена' }, period: 'PIE', family: 'ie' },
]
...
export function buildGraph() {
  ...
  COGNATE_SETS.forEach((cs, ci) => {
    ...
    nodes.push({ id: rootId, word: cs.root, language: 'Proto-IE', meaning: cs.meaning, family: 'ie', era: '公元前5000年', rootId: ci })
    Object.entries(cs.languages).forEach(([lang, word]) => {
      if (!word || word === '-') return
      const nid = ci + '_' + lang
      nodes.push({ id: nid, word, language: lang, meaning: cs.meaning, family: 'ie', era: '现代', rootId: ci })
